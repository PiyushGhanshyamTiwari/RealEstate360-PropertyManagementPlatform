...
public class LedgerEntryController {
	...
	@Operation(summary = "Fetching a ledger entry by id")
	@GetMapping("/{ledgerEntryId}")
	@PreAuthorize("hasAnyRole('ACCOUNT OFFICER','ADMIN')")
	public ResponseEntity<?> getLedgerEntryById(@PathVariable int ledgerEntryId) {
		LedgerEntryOutputDto response = ledgerEntryService.getLedgerEntryById(ledgerEntryId);
		return new ResponseEntity<>(response, HttpStatus.OK);
	}
